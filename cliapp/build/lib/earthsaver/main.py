...
class Code:
    ...
    def displayDiff(file_memory):
        ...
        def display(file_path):
            ...
            if Code.firsttime == 0:
                with open(f"{Code.current_index}json.txt", 'w', encoding='utf-8') as file:
                    for line in diff:
                        file.write(f"{line}\n")
                        Code.firsttime = 1
...
class Carbon:
    ...
    def dbPolling():
        ...
        def send_request(job_id):
            url = 'https://swe-team9.web.app/status_request'  # 실제 요청을 보낼 URL로 교체
            headers = {'Content-Type': 'application/json'}
...
def earthsaver(measure, refactor):
    ...
    with ThreadPoolExecutor() as executor:
        ...
        if measure:
            tasks.append(executor.submit(Carbon.setCarbonEmission, request_body))
        if refactor:
            tasks.append(executor.submit(Code.codeRefactoring, request_body))
        
        for task in tasks:
            task.result()

        if refactor:
            Code.displayDiff(file_memory)
        if measure:
            Carbon.emissionConvert()
